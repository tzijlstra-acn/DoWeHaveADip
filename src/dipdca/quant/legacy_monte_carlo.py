...
This module used ``run_wait_for_dip`` as the dip strategy, which uses the ETF
price (not the reference index) for the ATH/drawdown signal, resets the peak
inside rolling windows, and rearms on threshold recovery rather than ATH
recovery. Its observations (dates near a drawdown band) are not independent
ATH-episode crossings. It is retained only for reference; the replacement is
``dipdca.quant.ath_episodes`` (event study) and
``dipdca.quant.episode_bootstrap`` (paired episode bootstrap).
